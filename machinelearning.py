...
# Define Ridge model with hyperparameter grid
ridge_params = {'alpha': [0.1, 0.2, 0.5, 1.0, 2.0, 5.0, 10.0]}
ridge_model = Ridge()
...
# Tune the Ridge model and evaluate
best_ridge_model = tune_model(ridge_model, ridge_params, train, features)
mse, ap, combination = train_and_evaluate(best_ridge_model, train, test, features)
...
# Calculate average AP across multiple years for the Ridge model
...
avg_ap = evaluate_model_over_years(best_ridge_model, df, features)
print("Average AP over multiple years for the best Ridge model:")
print(avg_ap)
...
# Display the rankings with differences for the best Ridge model
